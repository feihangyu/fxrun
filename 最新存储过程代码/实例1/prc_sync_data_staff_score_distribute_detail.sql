CREATE DEFINER=`fedatasync`@`%` PROCEDURE `prc_sync_data_staff_score_distribute_detail`()
LABEL:BEGIN
...
SELECT count(*) into v_count FROM fe_dwd.dwd_staff_score_distribute_result;
IF v_count =0 THEN
...
SET autocommit=0;
DELETE a FROM fe_data.staff_score_distribute_detail a  
WHERE a.emp_user_id NOT IN 
(SELECT emp_user_id FROM  fe_dwd.dwd_staff_score_distribute_result);
...
END IF;  
REPEAT 
select min(emp_user_id) temp1,max(emp_user_id) temp2  into v_temp1,v_temp2  from (select emp_user_id from  fe_dwd.dwd_staff_score_distribute_result  where emp_user_id> v_temp2 limit v_interval) t ;
set v_tag=v_tag+v_interval;
#SET v_temp1=v_temp2;
#SET v_temp2=v_temp2+v_interval;
set v_error=0;
INSERT INTO fe_data.staff_score_distribute_detail(
emp_user_id,
group_customer_id,
job_number,        
distribute_amount
...
job_number,        
distribute_amount
FROM fe_dwd.dwd_staff_score_distribute_result 
  WHERE  emp_user_id>=v_temp1
  AND    emp_user_id<=v_temp2
ON DUPLICATE KEY UPDATE
emp_user_id=VALUES(emp_user_id),
group_customer_id=VALUES(group_customer_id),
job_number=VALUES(job_number),     
distribute_amount=VALUES(distribute_amount);
IF v_error=1 THEN  
        ROLLBACK; -- 事务回滚  
