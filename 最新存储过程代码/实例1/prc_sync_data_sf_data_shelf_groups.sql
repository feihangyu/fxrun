CREATE DEFINER=`fedatasync`@`%` PROCEDURE `prc_sync_data_sf_data_shelf_groups`()
LABEL:BEGIN
...
SELECT count(*) into v_count FROM feods.D_LO_data_shelf_groups ;
IF v_count =0 THEN
...
SET autocommit=0;
DELETE a FROM fe_data.sf_data_shelf_groups a  
WHERE a.groups_id NOT IN 
(SELECT groups_id FROM  feods.D_LO_data_shelf_groups);
...
END IF;  
REPEAT 
select min(groups_id) temp1,max(groups_id) temp2  into v_temp1,v_temp2  from (select groups_id from  feods.D_LO_data_shelf_groups  where groups_id> v_temp2 limit v_interval) t ;
set v_tag=v_tag+v_interval;
#SET v_temp1=v_temp2;
#SET v_temp2=v_temp2+v_interval;
set v_error=0;
start transaction;
INSERT INTO fe_data.sf_data_shelf_groups(
groups_id,
groups_name,
shelf_manager_id,
is_default_groups,
groups_center_latitude,
groups_center_longitude,
modify_groups_type,
data_flag,
add_time,
add_user_id,
...
last_update_user_id,
last_update_time
FROM feods.D_LO_data_shelf_groups 
  WHERE  groups_id>=v_temp1
  AND    groups_id<=v_temp2
ON DUPLICATE KEY UPDATE
groups_id=VALUES(groups_id),
groups_name=VALUES(groups_name),
shelf_manager_id=VALUES(shelf_manager_id),
is_default_groups=VALUES(is_default_groups),
groups_center_latitude=VALUES(groups_center_latitude),
groups_center_longitude=VALUES(groups_center_longitude),
modify_groups_type=VALUES(modify_groups_type),
