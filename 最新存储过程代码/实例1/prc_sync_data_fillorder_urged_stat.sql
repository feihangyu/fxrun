CREATE DEFINER=`fedatasync`@`%` PROCEDURE `prc_sync_data_fillorder_urged_stat`()
LABEL:BEGIN
...
SELECT count(*) into v_count FROM feods.D_LO_fillorder_urged_stat ;
IF v_count =0 THEN
...
SET autocommit=0;
DELETE a FROM fe_data.fillorder_urged_stat a  
WHERE a.stat_id NOT IN 
(SELECT stat_id FROM  feods.D_LO_fillorder_urged_stat);
...
END IF;  
REPEAT 
select min(stat_id) temp1,max(stat_id) temp2  into v_temp1,v_temp2  from (select stat_id from  feods.D_LO_fillorder_urged_stat  where stat_id> v_temp2 limit v_interval) t ;
set v_tag=v_tag+v_interval;
#SET v_temp1=v_temp2;
#SET v_temp2=v_temp2+v_interval;
set v_error=0;
start transaction;
INSERT INTO fe_data.fillorder_urged_stat(
stat_id,
shelf_manager_id,
shelf_id,
stat_date,
...
last_update_time,
last_update_user_id
FROM feods.D_LO_fillorder_urged_stat 
  WHERE  stat_id>=v_temp1
  AND    stat_id<=v_temp2
