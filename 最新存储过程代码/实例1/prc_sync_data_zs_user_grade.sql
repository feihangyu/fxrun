CREATE DEFINER=`fedatasync`@`%` PROCEDURE `prc_sync_data_zs_user_grade`()
LABEL:BEGIN
...
SELECT count(*) into v_count FROM feods.zs_user_grade ;
IF v_count =0 THEN
...
SET autocommit=0;
DELETE a FROM fe_data.zs_user_grade a  
WHERE a.USER_GRADE_ID NOT IN 
(SELECT USER_GRADE_ID FROM  feods.zs_user_grade);
...
END IF;  
REPEAT 
select min(user_grade_id) temp1,max(user_grade_id) temp2  into v_temp1,v_temp2  from (select user_grade_id from  feods.zs_user_grade  where user_grade_id> v_temp2 limit v_interval) t ;
set v_tag=v_tag+v_interval;
#SET v_temp1=v_temp2;
#SET v_temp2=v_temp2+v_interval;
set v_error=0;
INSERT INTO fe_data.zs_user_grade(
  user_grade_id,
  user_id,
  grade,
  add_time,
...
FROM
  feods.zs_user_grade
  WHERE  user_grade_id>=v_temp1
  AND    user_grade_id<=v_temp2  
 ON DUPLICATE KEY UPDATE 
  user_grade_id=VALUES(user_grade_id),
