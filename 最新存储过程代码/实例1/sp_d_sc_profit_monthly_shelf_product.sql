CREATE DEFINER=`feprocess`@`%` PROCEDURE `sp_d_sc_profit_monthly_shelf_product`()
BEGIN
-- =============================================
-- Author:	wuting
...
# 1、当月GMV统计
DELETE FROM feods.`d_sc_profit_monthly_shelf_product` WHERE stat_month = DATE_FORMAT(SUBDATE(CURDATE(),1),"%Y-%m");
INSERT INTO feods.`d_sc_profit_monthly_shelf_product`
( stat_month,
  `region_area`,
  `business_area`,
  `shelf_type`,
...
WHERE order_date >= DATE_FORMAT(SUBDATE(CURDATE(),1),"%Y-%m-01") AND order_date < CURDATE()
GROUP BY 
a.product_id
-- ,DATE(a.order_date)
, s.business_name
...
'sp_d_sc_profit_monthly_shelf_product',
