CREATE DEFINER=`feprocess`@`%` PROCEDURE `sp_op_product_area_disrate`()
BEGIN
  ...
  CREATE TEMPORARY TABLE feods.sal_tmp (
    PRIMARY KEY (product_id, business_name)
  )
  SELECT
  ...
  TRUNCATE feods.d_op_product_area_disrate;
  INSERT INTO feods.d_op_product_area_disrate (
...
-- ⑥    架上周转天数＜30,9折
-- ⑦    架上周转天数显示空/无，4折
   TRUNCATE feods.d_op_sp_disrate;
  INSERT INTO feods.d_op_sp_disrate (
  ...
  FROM
    feods.d_op_product_area_disrate t
    JOIN feods.shelf_tmp s
  ...
  CALL feods.sp_task_log (
    'sp_op_product_area_disrate', @sdate, CONCAT(
      'fjr_d_9871b283d6fa42333c060fe89435f4fa', @timestamp, @add_user
    )
  );
  COMMIT;
END