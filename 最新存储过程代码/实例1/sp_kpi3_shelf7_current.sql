CREATE DEFINER=`feprocess`@`%` PROCEDURE `sp_kpi3_shelf7_current`()
BEGIN
  ...
  DELETE
  FROM
    feods.fjr_kpi3_shelf7_slot_stock_day
  WHERE sdate = @sdate;
  INSERT INTO feods.fjr_kpi3_shelf7_slot_stock_day (
  ...
  DELETE
  FROM
    feods.fjr_kpi3_shelf7_slot_sale_day
  WHERE sdate = @sdate;
  INSERT INTO feods.fjr_kpi3_shelf7_slot_sale_day (
  ...
  FROM
    fe.sf_shelf_slot_stock_record t
    LEFT JOIN feods.price_tmp p
      ON t.shelf_id = p.shelf_id
      AND t.product_id = p.product_id
  WHERE t.data_flag = 1
  ...
  DELETE
  FROM
    feods.fjr_kpi3_shelf7_shelf_stock_day
  WHERE sdate = @sdate;
  INSERT INTO feods.fjr_kpi3_shelf7_shelf_stock_day (
    sdate, shelf_id, sales_flag0, new_flag0, stock_quantity, stock_val, pskus, nskus, pstock_quantity, pstock_val, add_user
  )
  SELECT
    @sdate sdate, t.shelf_id, IFNULL(f.sales_flag, 0) sales_flag0, IFNULL(f.new_flag, 0) = 1 new_flag0, SUM(t.stock_quantity) stock_quantity, SUM(t.stock_quantity * t.sale_price) stock_val, SUM(t.stock_quantity > 0) pskus, SUM(t.stock_quantity < 0) nskus, SUM(
      IF(
  ...
  FROM
    feods.fjr_kpi3_shelf7_slot_stock_day t
    JOIN feods.shelf_tmp s
      ON t.shelf_id = s.shelf_id
      AND ! is_test
  WHERE t.sdate = @sdate;
  ...
  FROM
    feods.fjr_kpi3_shelf7_slot_sale_day t
    JOIN feods.shelf_tmp s
      ON t.shelf_id = s.shelf_id
      AND ! is_test
  WHERE t.sdate = @sdate;
  ...
  FROM
    feods.fjr_kpi3_shelf7_shelf_stock_day t
    JOIN feods.shelf_tmp s
      ON t.shelf_id = s.shelf_id
      AND ! is_test
  WHERE t.sdate = @sdate;
  ...
  SELECT
    @sql_str := CONCAT(
      "INSERT INTO feods.monitor_tmp(indicate_type,indicate_name,indicate_value)VALUES", "('d','货道动销率',@slots_sale / @slots),", "('d','严重滞销金额占比',@flag5_rate_d),('w','严重滞销金额占比',@flag5_rate_w),('m','严重滞销金额占比',@flag5_rate_m),", "('d','缺货货道率',1- @pslots / @slots);"
    );
  ...
  CALL feods.sp_task_log (
    'sp_kpi3_shelf7_current', @sdate, CONCAT(
      'fjr_d_0297a5834f3f7ada020a6b91e1751adb', @timestamp, @add_user
    )
  );
  CALL sh_process.sp_op_offstock_slot ();
  COMMIT;
END