CREATE DEFINER=`feprocess`@`%` PROCEDURE `sh_preware_outbound_fill`()
...
BEGIN
    ...
    SET l_task_name = 'sh_preware_outbound_fill'; 
SET @run_date := CURRENT_DATE(),@user := CURRENT_USER(),@timestamp := CURRENT_TIMESTAMP();
 # 前置仓每日出库数据，因为出库的状态在一个补货周期内会有变化，因此每次更新10天的数据；
SET @sdate = SUBDATE(CURDATE(),1);
DELETE FROM feods.preware_outbound_daily WHERE out_date >= DATE_SUB(CURDATE(),INTERVAL 17 DAY);
INSERT INTO feods.preware_outbound_daily
(out_date,
region_area,
business_area,
warehouse_number,
...
FROM
(SELECT CASE WHEN b.fill_type IN (9,10) THEN DATE(b.apply_time)
WHEN b.fill_type IN (1,2,8,15) AND b.send_time IS NOT NULL THEN DATE(b.send_time)
WHEN b.fill_type IN (1,2,8,15) AND b.send_time IS NULL THEN DATE(b.apply_time)
END AS 'adjust_send_time',
IF(b.fill_type = 15,b.shelf_id,b.supplier_id) AS supplier_id_adjust,
a.product_id,
b.fill_type,
...
LEFT JOIN fe.`sf_product_fill_order_extend` ex
...
((b.supplier_type = 9 AND b.fill_type IN (1,2,8,9,10) AND b.order_status IN (2,3,4)) 
OR (b.supplier_type = 2 AND b.fill_type = 15 AND b.order_status = 8))
AND a.data_flag =1
AND b.data_flag =1
AND b.apply_time >= DATE_SUB(CURDATE(),INTERVAL 40 DAY)
...
;
#前置仓每周出库，同样更新最近1周的数据
DELETE FROM feods.preware_outbound_weekly WHERE week_monday >= DATE_SUB(@sdate,INTERVAL WEEKDAY(@sdate)+7 DAY); #上周及本周的数据更新；
INSERT INTO feods.preware_outbound_weekly
(week_monday,
out_week,
region_area,
business_area,
...
FROM feods.preware_outbound_daily
...
,fill_type;
#前置仓每月出库
DELETE FROM feods.preware_outbound_monthly WHERE out_month = DATE_FORMAT(@sdate,'%Y-%m');
INSERT INTO feods.preware_outbound_monthly
(out_month,
region_area,
business_area,
warehouse_number,
...
DELETE FROM feods.preware_fill_daily WHERE fill_date >= DATE_SUB(CURDATE(),INTERVAL 14 DAY);
INSERT INTO feods.preware_fill_daily
(fill_date,
region_area,
business_area,
warehouse_number,
...
WHERE b.shelf_id  IN (SELECT DISTINCT warehouse_id FROM fe.sf_prewarehouse_stock_detail WHERE data_flag =1)
  AND b.fill_type IN (1,2,4,8,10)
  AND b.order_status IN (3,4)
  AND b.fill_time IS NOT NULL
  AND a.data_flag =1
  AND b.data_flag =1
  AND b.fill_time >= DATE_SUB(CURDATE(),INTERVAL 14 DAY) 
  AND b.fill_time < CURDATE()
GROUP BY 
  DATE(b.fill_time),
   b.shelf_id,
   b.supplier_id,
...
--         , ACTUAL_FILL_NUM
    FROM
        ...
        JOIN fe.sf_product_fill_order b
            ...
            AND b.fill_time >= DATE_SUB(CURDATE(),INTERVAL 14 DAY) 
            AND b.fill_time < CURDATE()
         JOIN fe.sf_shelf_goods_transfer t
            ON b.ORDER_ID = t.TARGET_ORDER_ID
            AND t.`DATA_FLAG` = 1
        JOIN fe.`sf_shelf` s
            ON t.SOURCE_SHELF_ID = s.shelf_id
            AND s.data_flag = 1
   GROUP BY DATE(b.fill_time),
   b.shelf_id,
   t.SOURCE_SHELF_ID,
   b.fill_type,
   a.product_id
...
AND w.to_preware = 1
LEFT JOIN feods.`zs_product_dim_sserp` p 
ON t4.business_name = p.business_area
AND fi.product_id = p.product_id
LEFT JOIN fe.`sf_shelf_product_detail_flag` sf
ON fi.supplier_id = sf.shelf_id
AND fi.product_id = sf.product_id
AND fi.fill_type = 12
AND sf.data_flag = 1;
# 已更新到2019年的近14天出库量（包括当天）
DELETE FROM feods.preware_outbound_forteen_day WHERE sdate = @sdate;
INSERT INTO feods.preware_outbound_forteen_day
(sdate,
region_area,
business_area,
warehouse_number,
...
FROM feods.preware_outbound_daily f2 
LEFT JOIN 
...
;
#近7天出库
DELETE FROM feods.d_sc_preware_outbound_seven_day WHERE sdate = @sdate;
INSERT INTO feods.d_sc_preware_outbound_seven_day
(sdate,
region_area,
business_area,
warehouse_number,
...
;
# 近3天出库
DELETE FROM feods.d_sc_preware_outbound_three_day WHERE sdate = @sdate;
INSERT INTO feods.d_sc_preware_outbound_three_day
(sdate,
region_area,
business_area,
warehouse_number,
...
GROUP BY f2.warehouse_id,f2.product_id; 
#近7天入库
DELETE FROM feods.d_sc_preware_fill_seven_day WHERE sdate = @sdate;
INSERT INTO feods.d_sc_preware_fill_seven_day
(sdate,
region_area,
business_area,
warehouse_number,
...
FROM feods.`preware_fill_daily` f2
