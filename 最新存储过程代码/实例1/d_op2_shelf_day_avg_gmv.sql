CREATE DEFINER=`feprocess`@`%` PROCEDURE `d_op2_shelf_day_avg_gmv`()
BEGIN
    ...
    DELETE FROM feods.d_op2_shelf_day_avg_gmv
    WHERE month_id = @month_id;
    
    INSERT INTO feods.d_op2_shelf_day_avg_gmv 
    SELECT @month_id,
           s.business_name,
           s.shelf_id,
           ...
           (SELECT COUNT(CASE WHEN if_work_day = 1 THEN sdate END) + 0.5*COUNT(CASE WHEN if_work_day = 0 AND DAYOFWEEK(sdate) = 7 AND holiday = '' THEN sdate END) + 0.4*COUNT(CASE WHEN if_work_day = 0 AND DAYOFWEEK(sdate) = 1 AND holiday = '' THEN sdate END) + 0.4*COUNT(CASE WHEN holiday != '' THEN sdate END)work_day FROM feods.fjr_work_days WHERE sdate >= @month_first_day AND sdate < @month_last_day) days,
           (IFNULL(after_pay.payment_money,0) + IFNULL(sale.gmv,0)) / (SELECT COUNT(CASE WHEN if_work_day = 1 THEN sdate END) + 0.5*COUNT(CASE WHEN if_work_day = 0 AND DAYOFWEEK(sdate) = 7 AND holiday = '' THEN sdate END) + 0.4*COUNT(CASE WHEN if_work_day = 0 AND DAYOFWEEK(sdate) = 1 AND holiday = '' THEN sdate END) + 0.4*COUNT(CASE WHEN holiday != '' THEN sdate END)work_day FROM feods.fjr_work_days WHERE sdate >= @month_first_day AND sdate < @month_last_day) day_gmv,
    ...
    LEFT JOIN -- 货架补付款
    (SELECT shelf_id,
            SUM(IFNULL(payment_money,0)) payment_money
