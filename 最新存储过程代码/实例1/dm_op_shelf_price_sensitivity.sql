CREATE DEFINER=`feprocess`@`%` PROCEDURE `dm_op_shelf_price_sensitivity`()
BEGIN
...
LEFT JOIN
(SELECT m.member_id,
        IF(m.sex IS NULL,w.sex,m.sex)sex
 FROM fe.pub_member m
 LEFT JOIN fe.pub_user_wechat w ON m.member_id = w.user_id
