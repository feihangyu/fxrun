CREATE DEFINER=`feprocess`@`%` PROCEDURE `d_op_shelf_type_product_sale`()
BEGIN
...
DELETE FROM feods.`d_op_shelf_type_product_sale` WHERE week_end=@week_end;	   
	   
-- 货架信息
DROP TEMPORARY TABLE IF EXISTS feods.shelf_type_tmp;
CREATE TEMPORARY TABLE feods.shelf_type_tmp (PRIMARY KEY (shelf_id)) AS
...
ON feods.shelf_type_tmp (shelf_id);
...
JOIN feods.shelf_type_tmp s ON a.shelf_id = s.shelf_id
...
JOIN feods.shelf_type_tmp s ON t.shelf_id = s.shelf_id
GROUP BY s.business_name,s.shelf_type,t.product_id;
-- 各种货架类型的周商品动销率
INSERT INTO feods.d_op_shelf_type_product_sale
(week_end
,business_name
,shelf_type
,product_id
