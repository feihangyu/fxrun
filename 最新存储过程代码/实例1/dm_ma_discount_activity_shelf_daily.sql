CREATE DEFINER=`feprocess`@`%` PROCEDURE `dm_ma_discount_activity_shelf_daily`(IN p_sdate DATE,IN p_update_flag TINYINT)
BEGIN
...
SET @update_flag=
    CASE WHEN p_update_flag>0 THEN p_update_flag
        WHEN CURRENT_TIME BETWEEN '07:20' AND '07:35' THEN 1
        WHEN CURRENT_TIME BETWEEN '11:00:00' AND '16:10:00' THEN 2
        ELSE 0 END ;
...
#删除数据
DELETE FROM fe_dm.dm_ma_discount_activity_shelf_daily WHERE sdate=@date0 OR sdate<SUBDATE(@date0,92);
#插入数据
DROP TEMPORARY TABLE IF EXISTS fe_dm.temp_stock1; #库存
CREATE TEMPORARY TABLE fe_dm.temp_stock1(INDEX(shelf_id,product_id)) AS
    SELECT shelf_id,product_id,sales_flag,stock_quantity,sale_price FROM fe_dwd.dwd_shelf_product_day_all_recent_32 WHERE sdate=SUBDATE(@date0,1);

DROP TEMPORARY TABLE IF EXISTS fe_dm.temp_shelf_product_configure; #配置货架商品
CREATE TEMPORARY TABLE  fe_dm.temp_shelf_product_configure(INDEX(shelf_id,product_id)) AS
...
;
DROP TEMPORARY TABLE IF EXISTS fe_dm.temp_stock; #当日库存
CREATE TEMPORARY TABLE fe_dm.temp_stock(INDEX(shelf_id,activity_id,sales_flag)) AS
    SELECT a1.shelf_id ,a1.activity_id,IFNULL(a3.sales_flag,0) sales_flag
        ,COUNT(1) shelf_product_configure,SUM(stock_quantity) stock,SUM(stock_quantity*sale_price) stock_value
    FROM fe_dm.temp_shelf_product_configure a1
    JOIN fe_dwd.dwd_shelf_base_day_all a2 ON a2.shelf_id=a1.shelf_id
    JOIN fe_dm.temp_stock1 a3 ON a3.shelf_id=a1.shelf_id AND a3.product_id=a1.product_id
    GROUP BY a1.shelf_id,a1.activity_id,a3.sales_flag;
...
CREATE TEMPORARY TABLE fe_dm.temp_activity_order(INDEX(ORDER_ID,product_id),INDEX(SHELF_ID,product_id))  AS # 当日优惠订单明细
    ...
    WHERE a1.PAY_DATE>=@date0 AND a1.PAY_DATE<@date1
        AND a1.ORDER_STATUS=2
    ;
DROP TEMPORARY TABLE IF EXISTS fe_dm.temp_sale;
CREATE TEMPORARY TABLE fe_dm.temp_sale(INDEX(shelf_id,activity_id,sales_flag),INDEX(ACTIVITY_ID))  AS # 当日优惠订单明细
    SELECT a2.shelf_id,a1.ACTIVITY_ID,IFNULL(a3.SALES_FLAG,0) sales_flag
    ...
    JOIN fe_dwd.dwd_shelf_base_day_all a4 ON a4.shelf_id=a2.shelf_id
        LEFT JOIN fe_dm.temp_stock1 a3 ON a3.SHELF_ID=a1.SHELF_ID AND a3.PRODUCT_ID=a1.product_id
    GROUP BY a2.shelf_id,a1.ACTIVITY_ID,a3.sales_flag;
...
INSERT INTO fe_dm.dm_ma_discount_activity_shelf_daily
    (sdate, shelf_id, sales_flag, activity_id, activity_name,cost_dept
...
SELECT @date0 sdate, t3.shelf_id, t3.sales_flag, t3.activity_id, t1.activity_name,cost_dept
    ,t1.business_type, discount_type, discount_value, full_piece_number, sku_type, restrictions_type, start_time, end_time
    ,shelf_product_configure,stock, stock_value, COMBINED_PRICE, quantity_act, shelf_product_sale,shelfs_sale
FROM fe_dm.temp_sale t3
LEFT JOIN fe_dm.temp_stock t2 ON  t2.shelf_id=t3.shelf_id AND t2.ACTIVITY_ID=t3.activity_id AND t2.sales_flag=t3.sales_flag
LEFT JOIN
...
#插入餐卡支付优惠,招行满减活动
INSERT INTO fe_dm.dm_ma_discount_activity_shelf_daily
    (sdate, shelf_id, sales_flag, activity_id
    , activity_name,cost_dept
    , COMBINED_PRICE, quantity_act, shelf_product_sale,shelfs_sale)
SELECT @date0 sdate,shelf_id,0 sales_flag,activity_id
...
GROUP BY shelf_id,ACTIVITY_ID;
END IF;
#记录日志
CALL sh_process.`sp_sf_dw_task_log`('dm_ma_discount_activity_shelf_daily',DATE_FORMAT(@run_date,'%Y-%m-%d'),CONCAT('纪伟铨@',@user,@timestamp));
END