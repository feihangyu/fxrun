CREATE DEFINER=`feprocess`@`%` PROCEDURE `prc_dm_ma_coupon_bi_daily`(IN p_sdate DATE,IN p_update_flag TINYINT)
BEGIN
    #每4小时运行一次,7:02更新昨天起的数据(1), 11:02到16:02更新今天数据(2)
SET @run_date:= CURRENT_DATE(), @user := CURRENT_USER(), @timestamp := CURRENT_TIMESTAMP();
SET @sdate=p_sdate;
SET @update_flag=
    CASE WHEN p_update_flag>0 THEN p_update_flag
        WHEN CURRENT_TIME BETWEEN '07:00' AND '07:05' THEN 1
        WHEN CURRENT_TIME BETWEEN '11:00:00' AND '16:10:00' THEN 2
        ELSE 0 END ;
SET @date0=CASE @update_flag WHEN 1 THEN SUBDATE(@sdate,1) WHEN 2 THEN @sdate else null END;
SET @date1=ADDDATE(@date0,1);
IF @update_flag>0 THEN #在限定时段才运行
#删除数据
DELETE FROM feods.dm_ma_coupon_bi_daily WHERE sdate=@date0 OR sdate<SUBDATE(@date0,60);
#插入数据
INSERT INTO feods.dm_ma_coupon_bi_daily
    (sdate, CITY_NAME, coupon_id, coupon_name
...
SELECT @date0 sdate,IFNULL(a3.CITY_NAME,'无领取货架') city_name
...
JOIN fe.sf_coupon_scope_delivery a11 ON a11.coupon_id=a1.coupon_id and a11.data_flag=1
JOIN fe.sf_coupon_use a2 ON a2.coupon_id=a1.coupon_id and a2.data_flag=1
...
GROUP BY a3.CITY_NAME, a1.coupon_id
;
END IF;
-- 记录日志
CALL sh_process.`sp_sf_dw_task_log`('prc_dm_ma_coupon_bi_daily',DATE_FORMAT(@run_date,'%Y-%m-%d'),CONCAT('纪伟铨@',@user,@timestamp));
END