CREATE DEFINER=`feprocess`@`%` PROCEDURE `sp_d_mp_boss_data_shelf6`(in_sdate DATE)
...
BEGIN
  ...
  DELETE
  FROM
    feods.d_mp_day_sale_shelf6
  WHERE sdate = @sdate;
  INSERT INTO feods.d_mp_day_sale_shelf6(
    sdate,
    gmv,
    buy_qty,
    amount,
...
--       AND s.shelf_status IN (2, 4, 5)) shelfs_jg,
  ...
  DELETE
  FROM
    feods.d_mp_user_shelf6;
  INSERT INTO feods.d_mp_user_shelf6 (
    curweek,
    lastweek,
    lastweek_all,
    curmonth,
  ...
  DELETE
  FROM
    feods.d_mp_boss_data_shelf6
  WHERE report_day = @sdate;
  INSERT INTO feods.d_mp_boss_data_shelf6(
    report_day,
    row_num,
    stime,
    sdate,
...
--           SUM(a.shelfs_jg) shelfs_jg
        ...
        WHERE a.sdate >= SUBDATE(@sdate, WEEKDAY(@sdate) + 7)
          AND a.sdate < SUBDATE(@sdate, WEEKDAY(@sdate))
        GROUP BY '上周累计') t1
        LEFT JOIN feods.d_mp_user_shelf6 t2
          ON 1
      UNION
      ALL
      SELECT
