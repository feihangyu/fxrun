CREATE DEFINER=`feprocess`@`%` PROCEDURE `sp_d_sc_preware_balance`(in_sdate datetime)
    SQL SECURITY INVOKER
BEGIN
SET @sdate = in_sdate;
SET @sdate1 = DATE_SUB(in_sdate,INTERVAL 1 DAY);
SET @run_date := CURRENT_DATE(),@user := CURRENT_USER(),@timestamp := CURRENT_TIMESTAMP();
DELETE FROM feods.d_sc_preware_balance WHERE sdate = @sdate;
INSERT INTO feods.d_sc_preware_balance 
(sdate,
region_area,
business_area,
warehouse_number,
...
FROM feods.preware_outbound_daily t
...
(SELECT t.fill_date,t.warehouse_id,t.shelf_name,t.shelf_code,t.product_id,SUM(IFNULL(actual_fill_num,0)) AS actual_fill_num
 FROM feods.preware_fill_daily t 
...
LEFT JOIN feods.preware_outbound_forteen_day t5# 14天出库量
ON t.warehouse_id = t5.warehouse_id
AND t.product_id = t5.product_id
AND t.check_date = t5.sdate
AND t5.sdate = @sdate
LEFT JOIN feods.`d_sc_preware_sales_daily` t6
