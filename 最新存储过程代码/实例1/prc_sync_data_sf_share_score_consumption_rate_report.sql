CREATE DEFINER=`fedatasync`@`%` PROCEDURE `prc_sync_data_sf_share_score_consumption_rate_report`()
LABEL:BEGIN
...
SELECT count(*) into v_count FROM fe_dwd.dwd_en_fx_consumption_rate_report;
IF v_count =0 THEN
...
SET autocommit=0;
DELETE a FROM fe_data.sf_share_score_consumption_rate_report a  
WHERE a.id NOT IN 
(SELECT id FROM  fe_dwd.dwd_en_fx_consumption_rate_report);
...
END IF;  
REPEAT 
select min(id) temp1,max(id) temp2  into v_temp1,v_temp2  from (select id from  fe_dwd.dwd_en_fx_consumption_rate_report  where id> v_temp2 limit v_interval) t ;
set v_tag=v_tag+v_interval;
#SET v_temp1=v_temp2;
#SET v_temp2=v_temp2+v_interval;
set v_error=0;
INSERT INTO fe_data.sf_share_score_consumption_rate_report(
id,
auth_id,
auth_create_time,
report_time,
...
total_consume_score,
score_consume_rate,
score_consume_relate_rate,
total_receive_person,
total_consume_person,
person_consume_rate,
person_consume_relate_rate,
data_flag,
add_time,
add_user_id,
last_update_time,
...
last_update_time,
last_update_user_id
FROM fe_dwd.dwd_en_fx_consumption_rate_report 
  WHERE  id>=v_temp1
  AND    id<=v_temp2
...
total_consume_score=VALUES(total_consume_score),
score_consume_rate=VALUES(score_consume_rate),
score_consume_relate_rate=VALUES(score_consume_relate_rate),
total_receive_person=VALUES(total_receive_person),
total_consume_person=VALUES(total_consume_person),
person_consume_rate=VALUES(person_consume_rate),
person_consume_relate_rate=VALUES(person_consume_relate_rate),
