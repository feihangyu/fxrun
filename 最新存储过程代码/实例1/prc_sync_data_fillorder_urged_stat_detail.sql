CREATE DEFINER=`fedatasync`@`%` PROCEDURE `prc_sync_data_fillorder_urged_stat_detail`()
LABEL:BEGIN
...
SELECT count(*) into v_count FROM feods.D_LO_fillorder_urged_stat_detail ;
IF v_count =0 THEN
...
SET autocommit=0;
DELETE a FROM fe_data.fillorder_urged_stat_detail a  
WHERE a.detail_id NOT IN 
(SELECT detail_id FROM  feods.D_LO_fillorder_urged_stat_detail);
...
END IF;  
REPEAT 
select min(detail_id) temp1,max(detail_id) temp2  into v_temp1,v_temp2  from (select detail_id from  feods.D_LO_fillorder_urged_stat_detail  where detail_id> v_temp2 limit v_interval) t ;
set v_tag=v_tag+v_interval;
#SET v_temp1=v_temp2;
#SET v_temp2=v_temp2+v_interval;
set v_error=0;
start transaction;
INSERT INTO fe_data.fillorder_urged_stat_detail(
detail_id,
stat_id,
product_id,
data_flag,
...
last_update_time,
last_update_user_id
FROM feods.D_LO_fillorder_urged_stat_detail 
  WHERE  detail_id>=v_temp1
  AND    detail_id<=v_temp2
ON DUPLICATE KEY UPDATE
detail_id=VALUES(detail_id), 
stat_id=VALUES(stat_id), 
