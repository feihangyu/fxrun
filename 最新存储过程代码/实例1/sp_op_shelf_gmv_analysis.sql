CREATE DEFINER=`feprocess`@`%` PROCEDURE `sp_op_shelf_gmv_analysis`()
...
BEGIN
        ...
        DECLARE l_task_name     VARCHAR(64);
                ...
                END; 
        SET l_task_name = 'sp_op_shelf_gmv_analysis';
...
-- 月维度货架等级、GMV 30s
DROP TEMPORARY TABLE IF EXISTS feods.`shelf_gmv_tmp`;   
CREATE TEMPORARY TABLE feods.shelf_gmv_tmp (
...
FROM
        feods.`d_op_product_shelf_stat` t
        LEFT JOIN feods.d_op_product_shelf_sal_month_large l
                ON t.product_id = l.product_id
                AND t.shelf_id = l.shelf_id
                AND  l.month_id = @cur_year_month
        LEFT JOIN feods.d_op_product_shelf_sal_month_large l1
...
;
-- 近7天GMV 1s
DROP TEMPORARY TABLE IF EXISTS feods.`shelf_gmv_day7`;   
CREATE TEMPORARY TABLE feods.shelf_gmv_day7 (
        KEY idx_shelf_id(shelf_id)
) AS
SELECT
        a.shelf_id,
...
-- 货架低库存上限 1s
DROP TEMPORARY TABLE IF EXISTS feods.`low_stock_upper_limit`;
CREATE TEMPORARY TABLE feods.low_stock_upper_limit(
        KEY idx_shelf_id(shelf_id)
) AS 
SELECT 
        a.shelf_id,
        CASE 
                WHEN c.grade IN ('丙','丁') AND a.shelf_type IN (1,3)
                        THEN IFNULL(b.shelf_stock_upper_limit,0) + 110
                WHEN c.grade IN ('丙','丁') AND a.shelf_type IN (2,5)
...
FROM
        ...
        LEFT JOIN 
                (
                        SELECT
                                ...
                                SUM(
                                        CASE
                                                WHEN c.grade IN ('丙','丁') AND b.shelf_type IN (1,3) THEN 110
                                                WHEN c.grade IN ('丙','丁') AND b.shelf_type IN (2,5) THEN 90
                                                WHEN b.shelf_type IN (1,3) THEN 180
                                                WHEN b.shelf_type IN (2,5) THEN 110
                                        END
                                ) AS shelf_stock_upper_limit
                        FROM
                                fe.sf_shelf_relation_record a
                                LEFT JOIN `fe_dwd`.`dwd_shelf_base_day_all` b
                                        ON a.`SECONDARY_SHELF_ID` = b.`SHELF_ID`
                                JOIN feods.`d_op_shelf_grade` c
                                        ON a.`SECONDARY_SHELF_ID` = c.shelf_id
                                        AND c.month_id = @pre_year_month
        ...
        JOIN feods.`d_op_shelf_grade` c
                ...
                AND c.month_id = @pre_year_month
;
DROP TEMPORARY TABLE IF EXISTS feods.`shelf_low_stock_tmp`;   
CREATE TEMPORARY TABLE feods.shelf_low_stock_tmp (
        KEY idx_shelf_id(shelf_id)
) AS
SELECT
        a.shelf_id,
...
CREATE TEMPORARY TABLE feods.check_tmp (
        KEY idx_shelf_id(shelf_id)
) AS
SELECT
        a.shelf_id,
...
;
-- 本周取消订单 1s 
DROP TEMPORARY TABLE IF EXISTS feods.`fill_off_tmp`;   
CREATE TEMPORARY TABLE feods.fill_off_tmp (
        KEY idx_shelf_id(shelf_id)
) AS
SELECT
        a.shelf_id,
...
CREATE TEMPORARY TABLE feods.sku_tmp (
        KEY idx_shelf_id(shelf_id)
) AS
SELECT
        a.shelf_id,
...
FROM
        ...
        JOIN `fe_dwd`.dwd_shelf_base_day_all b
                ON a.shelf_id = b.shelf_id
        LEFT JOIN feods.`zs_product_dim_sserp` c
...
;
-- 巡检表 2s
DROP TEMPORARY TABLE IF EXISTS feods.`inspection_tmp`;   
CREATE TEMPORARY TABLE feods.inspection_tmp (
        KEY idx_shelf_id(shelf_id)
) AS
SELECT
DISTINCT
...
FROM
  ...
  LEFT JOIN
    ...
    FROM
      ...
      FROM
        fe.sf_shelf_inspection_survey_answer t
        JOIN fe.sf_survey_question q
  ...
  LEFT JOIN
    (SELECT
      DATE(t.operate_time) operate_time,
      t.check_id,
      t.shelf_id,
      SUM(cd.error_num * cd.sale_price) error_val
    FROM
      fe.sf_shelf_check t
      JOIN
        (SELECT
        ...
        FROM
          fe.sf_shelf_check t
        WHERE t.data_flag = 1
          AND t.operate_time >= SUBDATE(CURRENT_DATE, DAY(CURRENT_DATE) - 1)
          AND t.check_type = 4
        GROUP BY t.shelf_id) tm
        ON t.check_id = tm.check_id
      JOIN fe.sf_shelf_check_detail cd
        ON t.check_id = cd.check_id
        AND cd.data_flag = 1
    GROUP BY t.check_id) ck
    ON mt.shelf_id = ck.shelf_id
  LEFT JOIN fe.sf_shelf_inspection_task_operation ito
    ON t.id = ito.task_id
    AND ito.data_flag = 1
...
;
-- 次日上架率 2s
DROP TEMPORARY TABLE IF EXISTS feods.`intime_tmp`;   
CREATE TEMPORARY TABLE feods.intime_tmp (
        KEY idx_shelf_id(shelf_id)
) AS
SELECT 
       shelf_id,
       IFNULL(COUNT(CASE WHEN two_days_fill_label = '及时' THEN order_id END),0) / COUNT(order_id) AS intime_rate -- 次日上架率
FROM feods.D_LO_shelf_fill_timeliness_detail
...
FROM
        `fe_dwd`.`dwd_shelf_base_day_all` a
        LEFT JOIN feods.shelf_gmv_tmp b
                ON a.`shelf_id` = b.shelf_id
        LEFT JOIN feods.`fjr_shelf_board` c
                ON a.`shelf_id` = c.shelf_id
        LEFT JOIN feods.shelf_low_stock_tmp d
