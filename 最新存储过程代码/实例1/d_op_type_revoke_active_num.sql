CREATE DEFINER=`feprocess`@`%` PROCEDURE `d_op_type_revoke_active_num`()
BEGIN
    SET  @month_id := DATE_FORMAT(SUBDATE(CURRENT_DATE,1),'%Y-%m');
    SET @run_date := CURRENT_DATE(),@user := CURRENT_USER(),@timestamp := CURRENT_TIMESTAMP();
DELETE FROM feods.d_op_type_revoke_active_num WHERE month_id = @month_id;	   
INSERT INTO feods.d_op_type_revoke_active_num 
(month_id      
,business_name 
,zone_name
,shelf_type    
...
LEFT JOIN  -- 撤架id
   ...
   ON s.shelf_id=a.shelf_id
    LEFT JOIN
 (SELECT tc.shelf_id
 FROM fe.`sf_shelf_logistics_task_change` tc 
 JOIN  fe.`sf_shelf_logistics_task` t ON tc.logistics_task_id=t.logistics_task_id
