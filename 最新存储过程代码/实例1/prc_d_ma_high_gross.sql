CREATE DEFINER=`feprocess`@`%` PROCEDURE `prc_d_ma_high_gross`()
BEGIN
...
/*失败重跑删掉数据*/
DELETE FROM feods.d_ma_high_gross WHERE sdate = @sdate;
/*货架商品标签-高毛利*/
DROP TABLE IF EXISTS `feods`.d_ma_valid_gross_shelf_temp;	
CREATE TEMPORARY TABLE `feods`.d_ma_valid_gross_shelf_temp(KEY(shelf_id),KEY(product_id))	
AS	
SELECT c.shelf_id,c.product_id, b.business_name	
FROM fe_dwd.`dwd_shelf_base_day_all` b
JOIN feods.`zs_shelf_product_flag` c ON c.shelf_id = b.shelf_id  #feods.zs_shelf_product_flag
...
;
INSERT INTO feods.d_ma_high_gross (sdate, business_name, GMV, QTY, DISCOUNT_AMOUNT,SALE_SHELF_NUM,on_shelf_stock_qty,on_shelf_stock_shelf_num, pay_aoumnt)
SELECT 
       DATE(a1.sdate) 日期,
       b1.business_name,
       SUM(a1.gmv) GMV,
...
JOIN `feods`.d_ma_valid_gross_shelf_temp b1 ON a1.shelf_id = b1.shelf_id AND a1.`product_id` = b1.product_id
WHERE 
