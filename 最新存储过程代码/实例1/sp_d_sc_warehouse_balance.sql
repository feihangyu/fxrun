CREATE DEFINER=`feprocess`@`%` PROCEDURE `sp_d_sc_warehouse_balance`(in_sdate DATE)
...
SET @sdate = in_sdate;
DROP TEMPORARY TABLE IF EXISTS feods.d_sc_warehouse_balance_out_tmp;
CREATE TEMPORARY TABLE feods.d_sc_warehouse_balance_out_tmp 
...
;
# 入库
DROP TEMPORARY TABLE IF EXISTS feods.d_sc_warehouse_balance_in_tmp;
CREATE TEMPORARY TABLE feods.d_sc_warehouse_balance_in_tmp 
(INDEX idx_warehouse_product(warehouse_number,product_code2))
SELECT 
...
HAVING qty > 0
;
DROP TEMPORARY TABLE IF EXISTS feods.d_sc_warehouse_balance_stock_tmp;
CREATE TEMPORARY TABLE feods.d_sc_warehouse_balance_stock_tmp 
(INDEX idx_warehouse_product(warehouse_number,product_code2))
...
;
DROP TEMPORARY TABLE IF EXISTS feods.d_sc_warehouse_balance_product_tmp;
CREATE TEMPORARY TABLE feods.d_sc_warehouse_balance_product_tmp 
SELECT t1.region_area,t1.business_area,t1.warehouse_number,t1.warehouse_name,t1.product_code2
FROM feods.d_sc_warehouse_balance_stock_tmp t1
...
SELECT @sdate
    ...
    , sa.sale_level
FROM
    feods.d_sc_warehouse_balance_product_tmp t1 
    LEFT JOIN feods.d_sc_warehouse_balance_stock_tmp s 
        ON t1.warehouse_number = s.warehouse_number 
        AND t1.product_code2 = s.product_code2 
    LEFT JOIN feods.d_sc_warehouse_balance_out_tmp ot 
        ON t1.warehouse_number = ot.warehouse_number 
        AND t1.product_code2 = ot.product_code2 
    LEFT JOIN feods.d_sc_warehouse_balance_in_tmp it 
    ...
    LEFT JOIN feods.zs_product_dim_sserp sp
