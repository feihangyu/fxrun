CREATE DEFINER=`fedatasync`@`%` PROCEDURE `prc_sync_data_stat_group_distribute_emp_rank`()
LABEL:BEGIN
...
SELECT count(*) into v_count FROM fe_dm.dm_en_group_distribute_emp_rank ;
IF v_count =0 THEN
...
SET autocommit=0;
DELETE a FROM fe_data.stat_group_distribute_emp_rank a  
WHERE a.emp_rank_id NOT IN 
(SELECT emp_rank_id FROM  fe_dm.dm_en_group_distribute_emp_rank);
...
END IF;  
REPEAT 
select min(emp_rank_id) temp1,max(emp_rank_id) temp2  into v_temp1,v_temp2  from (select emp_rank_id from  fe_dm.dm_en_group_distribute_emp_rank  where emp_rank_id> v_temp2 limit v_interval) t ;
set v_tag=v_tag+v_interval;
#SET v_temp1=v_temp2;
#SET v_temp2=v_temp2+v_interval;
set v_error=0;
INSERT INTO fe_data.stat_group_distribute_emp_rank(
emp_rank_id,
group_customer_id,
dept_id,
emp_id,
...
last_update_time,
last_update_user_id
FROM fe_dm.dm_en_group_distribute_emp_rank 
