CREATE DEFINER=`shprocess`@`%` PROCEDURE `dwd_lo_node_monitor_data_after_cleanout`()
BEGIN
-- =============================================
-- Author:	物流店主
...
-- =============================================
  ...
  DELETE
  FROM
    fe_dwd.dwd_lo_node_monitor_data_after_cleanout
  WHERE update_time = DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY);
  INSERT INTO fe_dwd.dwd_lo_node_monitor_data_after_cleanout(
    update_time,
    action_type,
    action_id,
    action_time,
  ...
  FROM
    ...
    WHERE a.`logTimeDate` >= DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY)
...
'dwd_lo_node_monitor_data_after_cleanout',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('蔡松林@', @user),
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dwd.dwd_lo_node_monitor_data_after_cleanout','dwd_lo_node_monitor_data_after_cleanout','蔡松林');
