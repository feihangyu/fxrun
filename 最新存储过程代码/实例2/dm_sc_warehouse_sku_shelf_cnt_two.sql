CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_sc_warehouse_sku_shelf_cnt_two`(in_sdate DATE)
BEGIN
-- =============================================
-- Author:	wuting
...
DROP TEMPORARY TABLE IF EXISTS fe_dm.dm_sc_warehouse_daily_tmp;
CREATE TEMPORARY TABLE fe_dm.dm_sc_warehouse_daily_tmp
AS 
SELECT 
    t1.FPRODUCEDATE
    ...
    WHEN t1.product_type IN("原有","新增（试运行）") AND (sa.sale_level IN ("非常不好卖","难卖","") OR ISNULL(sa.sale_level))THEN "难周转品" # 20200729将类型为""的类型加入计算
    ELSE "非正常品"    
    END AS stock_flag
...
--     WHEN t1.product_type IN("原有","新增（试运行）") AND (sa.sale_level IN ("非常不好卖","难卖","") OR ISNULL(sa.sale_level))THEN 3  # 20200729将类型为""的类型加入计算 
...
# ERP库存主表
JOIN fe_dwd.dwd_PJ_OUTSTOCK2_DAY t1
...
ON p.product_code2 = t1.product_bar
# 区域上架率
LEFT JOIN fe_dm.dm_warehouse_product_presence wp 
ON t1.FPRODUCEDATE = wp.FPRODUCEDATE
AND t1.warehouse_number = wp.warehouse_number
AND t1.product_bar = wp.product_bar
# BDP真实库存
LEFT JOIN fe_dwd.dwd_sc_bdp_warehouse_stock_daily s
ON  s.sdate = t1.FPRODUCEDATE
AND  s.warehouse = t1.warehouse_number
AND s.sku_no = t1.PRODUCT_BAR 
# t2 近14天出库 情况
LEFT JOIN fe_dm.dm_sc_warehouse_outbound_forteen_total t2 
...
ON t4.smonth = DATE_FORMAT(@sdate,"%Y-%m")
AND t4.business_area = t1.business_area 
AND t4.product_code2 = t1.product_bar 
# t5 本月累计出库
LEFT JOIN fe_dm.dm_sc_warehouse_outbound_monthly_total t5 
ON t5.smonth = DATE_FORMAT(@sdate,"%Y-%m")
AND t5.business_area = t1.business_area 
AND t5.product_code2 = t1.product_bar 
# sa(sale_area_level)商品清单
LEFT JOIN fe_dm.dm_area_product_sale_flag sa
ON  sa.business_area = t1.business_Area 
...
# t7 当日大仓在途
LEFT JOIN fe_dm.dm_sc_warehouse_onload t7
...
FROM fe_dm.dm_prewarehouse_stock_detail_monthly t
...
FROM fe_dwd.dwd_preware_outbound_monthly t
...
FROM fe_dm.dm_sc_preware_balance t
WHERE t.sdate = @sdate
GROUP BY t.business_area,t.product_code2
) t10
...
FROM fe_dwd.dwd_preware_outbound_forteen_day t
WHERE t.sdate = @sdate
GROUP BY t.business_area,t.product_code2
) t11
...
;
# 周转等级
DROP TEMPORARY TABLE IF EXISTS fe_dm.dm_sc_warehouse_daily_tmp2;
CREATE TEMPORARY TABLE fe_dm.dm_sc_warehouse_daily_tmp2
AS 
SELECT 
t1.*,
...
-- ELSE "无等级"
-- END AS turnover_level
FROM fe_dm.dm_sc_warehouse_daily_tmp t1
;
DELETE FROM fe_dm.dm_sc_warehouse_preware_stock_outbound WHERE sdate = @sdate;
INSERT INTO fe_dm.dm_sc_warehouse_preware_stock_outbound
(sdate,
  region_area,
  business_area,
  warehouse_name, 
...
END AS satisfy
FROM fe_dm.dm_sc_warehouse_daily_tmp2 t2
;
#(2) 加覆盖货架数量运营商品数量等
DELETE FROM fe_dm.dm_sc_warehouse_sku_shelf_cnt WHERE sdate = @sdate;
INSERT INTO fe_dm.dm_sc_warehouse_sku_shelf_cnt
(sdate,
 region_area,
  business_area,
  qzc_cnt,
...
COUNT(DISTINCT t2.prewarehouse_id) AS 'qzc_cnt',
COUNT(IF(t1.shelf_code NOT LIKE "QZC%",t1.shelf_id,NULL)) AS 'shelf_cnt',
COUNT(IF(t2.prewarehouse_id IS NOT NULL,t1.shelf_id,NULL)) AS 'qzc_shelf_cnt',
COUNT(IF(t2.prewarehouse_id IS NOT NULL,t1.shelf_id,NULL)) / COUNT(IF(t1.shelf_code NOT LIKE "QZC%",t1.shelf_id,NULL))  AS 'coverage_rate'
FROM fe_dwd.dwd_shelf_base_day_all t1
LEFT JOIN fe_dwd.dwd_relation_dc_prewarehouse_shelf_day_all t2
ON t1.shelf_id = t2.shelf_id
JOIN fe_dwd.dwd_city_business t3
ON t1.city = t3.city
...
'dm_sc_warehouse_sku_shelf_cnt_two',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('吴婷@', @user),
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_sc_warehouse_preware_stock_outbound','dm_sc_warehouse_sku_shelf_cnt_two','吴婷');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_sc_warehouse_sku_shelf_cnt','dm_sc_warehouse_sku_shelf_cnt_two','吴婷');
END