CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_op_shelf_product_fill_last_time2`()
BEGIN
...
SELECT 
    ...
    MIN(FILL_TIME) AS FILL_TIME_min,    
    ...
    SUBSTRING_INDEX(GROUP_CONCAT(ACTUAL_FILL_NUM ORDER BY fill_time ),',',1) AS ACTUAL_FILL_NUM_min,    
...
'dm_op_shelf_product_fill_last_time2',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('李世龙@', @user),
@stime);
-- 记录表的数据量
call sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_shelf_product_fill_last_time','dm_op_shelf_product_fill_last_time2','李世龙');
END