CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_mp_week_kpi_monitor`(in_sdate DATE)
BEGIN
...
WHEN a.`SHELF_TYPE` = 8 AND DATEDIFF(@sdate3,a.ACTIVATE_TIME) > 4 THEN a.`SHELF_ID`
END) t1
LEFT JOIN fe_dwd.dwd_shelf_product_stock_detail t2
ON t1.shelf_id = t2.shelf_id
...
FROM fe_dm.dm_mp_daily_shelf_stock_stag t
WHERE t.`sdate` >= @sdate1 AND t.`sdate` <= @sdate2
;
DELETE FROM fe_dm.dm_mp_week_kpi_monitor WHERE sdate = @sdate2;
INSERT INTO fe_dm.dm_mp_week_kpi_monitor
(`sdate` ,
  week_new_shelf ,
...
'dm_mp_week_kpi_monitor',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('吴婷@', @user),
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_mp_week_kpi_monitor','dm_mp_week_kpi_monitor','吴婷');
COMMIT;
    END