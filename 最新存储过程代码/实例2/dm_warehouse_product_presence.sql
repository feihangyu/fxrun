CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_warehouse_product_presence`()
BEGIN
-- =============================================
-- Author:	wuting
...
#(1)大仓商品上架率
DELETE FROM fe_dm.dm_warehouse_product_presence WHERE FPRODUCEDATE = DATE_SUB(CURDATE(), INTERVAL 1 DAY);
INSERT INTO fe_dm.dm_warehouse_product_presence
(fproducedate, 
big_area,
business_area ,
warehouse_number,
...
--     and s.whether_close = 2
    ...
    (SELECT s.business_name
     ...
     FROM fe_dm.dm_sc_shelf_packages pa
     LEFT JOIN fe_dm.dm_op_sp_avgsal30 st
...
'dm_warehouse_product_presence',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('吴婷@', @user),
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_warehouse_product_presence','dm_warehouse_product_presence','吴婷');
COMMIT;
    END