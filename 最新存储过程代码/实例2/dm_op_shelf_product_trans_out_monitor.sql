CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_op_shelf_product_trans_out_monitor`()
BEGIN
...
FROM
        ...
        LEFT JOIN fe_dwd.machine_tmp f
                ON a.shelf_id = f.shelf_id
                AND a.product_id = f.product_id           
GROUP BY a.shelf_id,a.product_id
;
-- 更新前天的数据
DROP TEMPORARY TABLE IF EXISTS fe_dwd.fill_pre_day2;
CREATE TEMPORARY TABLE fe_dwd.fill_pre_day2(      
        KEY idx_shelf_id_product_id(shelf_id,product_id)
 ) AS 
SELECT
        a.shelf_id,
...
WHERE a.apply_time >= @pre_day2
        ...
        AND a.ADD_USER_ID = 0
;
DROP TEMPORARY TABLE IF EXISTS fe_dwd.shelf_product_pre_day2;
CREATE TEMPORARY TABLE fe_dwd.shelf_product_pre_day2(      
        KEY idx_shelf_id_product_id(shelf_id,product_id)
 ) AS 
SELECT
        a.shelf_id,
...
FROM
        fe_dwd.fill_pre_day2 a
GROUP BY a.shelf_id,a.product_id
;
UPDATE
        fe_dm.dm_op_shelf_product_trans_out_monitor a
        JOIN fe_dwd.shelf_product_pre_day2 b
...
'dm_op_shelf_product_trans_out_monitor',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('宋英南@', @user),
@stime);
-- 记录表的数据量
call sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_shelf_product_trans_out_monitor','dm_op_shelf_product_trans_out_monitor','宋英南');
COMMIT;
	END