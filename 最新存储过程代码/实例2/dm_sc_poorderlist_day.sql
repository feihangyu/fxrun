CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_sc_poorderlist_day`()
BEGIN
SET @run_date := CURRENT_DATE(),@user := CURRENT_USER(),@stime := CURRENT_TIMESTAMP();
   
DROP TEMPORARY TABLE IF EXISTS  fe_dm.dm_sc_poorderlist_tmp;
CREATE TEMPORARY TABLE fe_dm.dm_sc_poorderlist_tmp
(KEY idx_fbillno (FBILLNO),
KEY idx_fbillno_material (FBILLNO,FMATERIALID),
KEY idx_warehouse_fnumber(DC_CODE,product_code2)
)
AS
SELECT
    b.FDATE , #AS '申请时间'
...
--     ) AS '采购申请金额',
   ...
   FROM
 fe_dwd.dwd_sserp_T_PUR_POORDERENTRY a  
 JOIN fe_dwd.dwd_sserp_T_PUR_POORDER b
 ON a.FID = b.FID
--  and b.FBILLNO = 'CGDD20200512067'
 AND b.FDATE >= SUBDATE(CURDATE(),INTERVAL 6 MONTH)
 LEFT JOIN fe_dwd.dwd_sserp_V_BD_BUYER_L c
 ON b.FPURCHASERID = c.fid
 JOIN fe_dwd.dwd_sserp_T_BD_SUPPLIER_L d
 ON b.FSUPPLIERID = d.FSUPPLIERID
 JOIN fe_dwd.dwd_sserp_T_BD_MATERIAL_L e
 ON a.FMATERIALID = e.FMATERIALID
 JOIN fe_dwd.dwd_sserp_T_BD_STOCK_L f
 ON b.F_BGJ_FSTOREHOUSE = f.FSTOCKID
 JOIN fe_dwd.dwd_sserp_T_BD_MATERIAL h
 ON a.FMATERIALID = h.FMATERIALID
 JOIN fe_dwd.dwd_sserp_T_BD_MATERIALGROUP_L i
 ON h.FMATERIALGROUP = i.fid
 JOIN fe_dwd.dwd_sserp_T_BD_STOCK j
 ON f.FSTOCKID = j.FSTOCKID
 LEFT JOIN fe_dwd.dwd_sserp_ZS_DC_BUSINESS_AREA k
 ON j.fnumber = k.DC_CODE
 JOIN fe_dwd.dwd_sserp_T_PUR_POORDERENTRY_F l
 ON a.fid = l.FID
 AND a.fentryid = l.FENTRYID
  GROUP BY b.FDATE,
    ...
    ;
#  通知收料 
DROP TEMPORARY TABLE IF EXISTS  fe_dm.dm_sc_poorderlist_receive_tmp;
CREATE TEMPORARY TABLE fe_dm.dm_sc_poorderlist_receive_tmp
...
SELECT
    ...
    FROM
      fe_dwd.dwd_sserp_T_PUR_RECEIVEENTRY a
      LEFT JOIN fe_dwd.dwd_sserp_T_PUR_RECEIVE b
        ON a.fid = b.fid
...
DROP TEMPORARY TABLE IF EXISTS  fe_dm.dm_sc_poorderlist_instock_tmp;
CREATE TEMPORARY TABLE fe_dm.dm_sc_poorderlist_instock_tmp
...
# 退料单
DROP TEMPORARY TABLE IF EXISTS  fe_dm.dm_sc_poorderlist_back1;
CREATE TEMPORARY TABLE fe_dm.dm_sc_poorderlist_back1
...
SELECT
    ...
    FROM
      fe_dwd.dwd_sserp_T_PUR_MRBENTRY a
      LEFT JOIN fe_dwd.dwd_sserp_T_PUR_MRB b
        ON a.fid = b.fid
    GROUP BY forderno,
      FMATERIALID
      -- ,a.FSEQ
;
    -- AND t1.FSEQ = t4.FSEQ
DROP TEMPORARY TABLE IF EXISTS  fe_dm.dm_sc_poorderlist_back2;
CREATE TEMPORARY TABLE fe_dm.dm_sc_poorderlist_back2
(KEY idx_forderno(forderno,FMATERIALID)) 
AS 
SELECT
      forderno,
    ...
    FROM
      fe_dwd.dwd_sserp_T_PUR_MRAPPENTRY a
      LEFT JOIN fe_dwd.dwd_sserp_T_PUR_MRAPP b
        ON a.fid = b.fid
    GROUP BY forderno,
      FMATERIALID
       -- ,FSEQ
...
# 下单前一天的库存情况
DROP TEMPORARY TABLE IF EXISTS  fe_dm.dm_sc_warehouse_stock_tmp;
CREATE TEMPORARY TABLE fe_dm.dm_sc_warehouse_stock_tmp
...
DELETE FROM  fe_dm.dm_sc_poorderlist_day WHERE purchase_time >= SUBDATE(CURDATE(),INTERVAL 6 MONTH);
INSERT INTO fe_dm.dm_sc_poorderlist_day
(
big_area,
business_area,
purchase_time,
...
LEFT JOIN fe_dwd.dwd_PJ_OUTSTOCK2_DAY m
ON m.warehouse_number = t1.DC_CODE   #仓库名称
AND m.PRODUCT_BAR = t1.product_code2
AND m.business_area = t1.business_area #关联FE码，商品ID在仓库表里面不全
AND m.FPRODUCEDATE = t3.FDATE #订货前一天的周转天数和正品库存量
;
 
# 当日仓库在途量
TRUNCATE fe_dm.dm_sc_warehouse_onload;
INSERT INTO fe_dm.dm_sc_warehouse_onload
(sdate,
region_area,
business_area,
warehouse_name,
...
FROM fe_dm.dm_sc_poorderlist_day pur
...
LEFT JOIN fe_dwd.dwd_PJ_OUTSTOCK2_DAY p          
ON p.warehouse_number = t.warehouse_number
AND p.product_bar = t.product_code2
AND p.business_area = t.business_area
AND FPRODUCEDATE >= DATE_SUB(CURDATE(),INTERVAL 1 DAY)
LEFT JOIN fe_dm.dm_sc_poorderlist_day pur
ON pur.warehouse_number = t.warehouse_number
...
'dm_sc_poorderlist_day',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('吴婷@', @user),
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_sc_poorderlist_day','dm_sc_poorderlist_day','吴婷');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_sc_warehouse_onload','dm_sc_poorderlist_day','吴婷');
COMMIT;
    END