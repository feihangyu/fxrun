CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_ma_shelf_user_stat`()
BEGIN
...
SET @sdate= SUBDATE(CURDATE(),1); SET @sweekend=ADDDATE(@sdate,IF(DAYOFWEEK(@sdate)=1,0,8-DAYOFWEEK(@sdate))+0);SET @smonthend=LAST_DAY(@sdate);
    #激活时间100天内的用户累计数据
DROP TEMPORARY TABLE IF EXISTS  fe_dm.temp_shelf_user_duration;
CREATE TEMPORARY TABLE  fe_dm.temp_shelf_user_duration(INDEX(SHELF_ID)) AS
SELECT SHELF_ID
...
FROM
    ...
    JOIN fe_dm.dm_op_su_stat a2 ON a1.SHELF_ID=a2.shelf_id
    WHERE a1.SHELF_STATUS NOT IN (10,1) AND a1.SHELF_TYPE NOT IN (9)
      AND DATEDIFF(@sdate,DATE(ACTIVATE_TIME))<100
    ) a1
GROUP BY SHELF_ID;
    #激活时间100天内的30天终端用户数
DROP TEMPORARY TABLE IF EXISTS fe_dm.temp_shelf_user_day;
CREATE TEMPORARY TABLE  fe_dm.temp_shelf_user_day(INDEX(SHELF_ID)) AS
    SELECT a1.shelf_id
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),0),a3.user_num,0))   users_day1
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),1),a3.user_num,0))   users_day2
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),2),a3.user_num,0))   users_day3
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),3),a3.user_num,0))   users_day4
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),4),a3.user_num,0))   users_day5
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),5),a3.user_num,0))   users_day6
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),6),a3.user_num,0))   users_day7
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),7),a3.user_num,0))   users_day8
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),8),a3.user_num,0))   users_day9
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),9),a3.user_num,0))   users_day10
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),10),a3.user_num,0))  users_day11
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),11),a3.user_num,0))  users_day12
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),12),a3.user_num,0))  users_day13
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),13),a3.user_num,0))  users_day14
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),14),a3.user_num,0))  users_day15
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),15),a3.user_num,0))  users_day16
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),16),a3.user_num,0))  users_day17
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),17),a3.user_num,0))  users_day18
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),18),a3.user_num,0))  users_day19
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),19),a3.user_num,0))  users_day20
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),20),a3.user_num,0))  users_day21
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),21),a3.user_num,0))  users_day22
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),22),a3.user_num,0))  users_day23
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),23),a3.user_num,0))  users_day24
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),24),a3.user_num,0))  users_day25
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),25),a3.user_num,0))  users_day26
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),26),a3.user_num,0))  users_day27
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),27),a3.user_num,0))  users_day28
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),28),a3.user_num,0))  users_day29
    ,SUM(IF(a3.sdate=ADDDATE(DATE(a1.ACTIVATE_TIME),29),a3.user_num,0))  users_day30
FROM fe_dwd.dwd_shelf_base_day_all a1
...
JOIN fe_dm.dm_ma_shelf_sale_daily a3 ON a3.sdate=a2.sdate AND a3.shelf_id=a1.shelf_id
WHERE  SHELF_STATUS  NOT IN (1,10)  AND  a1.shelf_type NOT IN (9)
      AND DATEDIFF(@sdate,DATE(ACTIVATE_TIME))<100
GROUP BY a1.shelf_id
;   # 插入数据
REPLACE INTO fe_dm.dm_ma_shelf_user_stat
    ( shelf_id
...
SELECT a1.shelf_id
     ...
     , user_num_week11, user_num_week12, user_num_week13
     , user_num_week14, user_num_month1, user_num_month2, user_num_month3
FROM fe_dm.temp_shelf_user_duration a1
LEFT JOIN fe_dm.temp_shelf_user_day a2 ON a2.shelf_id=a1.shelf_id
;
-- 执行记录日志
CALL sh_process.`sp_sf_dw_task_log` ('dm_ma_shelf_user_stat',DATE_FORMAT(@run_date, '%Y-%m-%d'),CONCAT('纪伟铨@', @user),@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_ma_shelf_user_stat','dm_ma_shelf_user_stat','纪伟铨');
END