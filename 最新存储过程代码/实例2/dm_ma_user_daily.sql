CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_ma_user_daily`(IN p_sdate DATE)
BEGIN
...
SET @sweek=SUBDATE(@sdate,IF(DAYOFWEEK(@sdate)=1,6,DAYOFWEEK(@sdate)-2)) #当周一
    ...
    ;
#删除历史数据
DELETE FROM fe_dm.dm_ma_user_daily WHERE sdate=@sdate OR sdate<SUBDATE(@sdate,380);
#临时数据
...
SELECT  USER_ID,bb.SHELF_TYPE,bb.business_name,COUNT(1) ordernum
    FROM
        ...
        WHERE a1.PAY_DATE >= @sdate  AND a1.PAY_DATE < ADDDATE(@sdate, 1)
...
INSERT INTO fe_dm.dm_ma_user_daily
...
SELECT @sdate sdate,t1.business_name
...
-- 执行记录日志
CALL sh_process.`sp_sf_dw_task_log` ('dm_ma_user_daily',DATE_FORMAT(@run_date, '%Y-%m-%d'),CONCAT('纪伟铨@', @user),@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_ma_user_daily','dm_ma_user_daily','纪伟铨');
END