CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_sc_preware_kpi`()
BEGIN
...
FROM fe_dwd.dwd_preware_outbound_forteen_day t1
...
FROM fe_dm.dm_prewarehouse_stock_detail_weekly t1
...
FROM fe_dm.dm_prewarehouse_stock_detail t1
WHERE t1.check_date = @sdate3
) m1
JOIN 
(SELECT a.business_area,a.product_fe,a.product_type
FROM fe_dwd.dwd_pub_product_dim_sserp_his a
...
fe_dwd.`dwd_product_base_day_all` m4
ON m1.product_code2 = m4.product_code2
...
-- LEFT JOIN fe_dm.dm_op_warehouse_monitor m
...
-- FROM fe_dm.dm_sc_preware_sku_satisfy s
...
(SELECT
 SUM(CASE 
  ...
  FROM fe_dm.dm_fill_order_efficiency e
JOIN
-- (SELECT DISTINCT FNUMBER,F_BGJ_FBOXEDSTANDARDS
-- FROM sserp.T_BD_MATERIAL) s
-- ON e.product_code2 = s.fnumber
fe_dwd.`dwd_product_base_day_all` s
ON e.product_code2 = s.product_code2
LEFT JOIN 
(SELECT DATE_ADD(t.sdate,INTERVAL 1 DAY ) AS sdate
, t.warehouse_id
, t.product_id
, t.suggest_fill_qty
FROM fe_dm.dm_sc_preware_daily_report t
...
(SELECT
 SUM(CASE 
  ...
  END)/COUNT(*) AS fill_rational_rate
  FROM fe_dm.dm_fill_order_efficiency e
  -- JOIN fe_dm.dm_preware_shelf_sales_thirty s
  JOIN fe_dm.dm_sc_preware_daily_report s #为了和过年期间调整后的日期和畅销等级一致，改用d_sc_preware_daily_report
...
FROM fe_dwd.dwd_preware_outbound_monthly
...
FROM fe_dwd.dwd_preware_fill_daily f
...
(SELECT
ROUND(COUNT(DISTINCT IF(a.fill_type = 2,a.order_id,NULL))/ COUNT(DISTINCT a.order_id),4) AS system_fill_rate
FROM fe_dwd.dwd_fill_day_inc a
WHERE a.supplier_type = 9
AND a.fill_type IN (2,8,9)
AND a.order_status IN (3,4)
AND 
CASE WHEN a.fill_type IN (9) THEN DATE(a.apply_time)
WHEN a.fill_type IN (2,8) AND a.send_time IS NOT NULL THEN DATE(a.send_time)
WHEN a.fill_type IN (2,8) AND a.send_time IS NULL THEN DATE(a.apply_time)
END >= @sdate2 
AND 
CASE WHEN a.fill_type IN (9) THEN DATE(a.apply_time)
WHEN a.fill_type IN (2,8) AND a.send_time IS NOT NULL THEN DATE(a.send_time)
...
# 低库存货架占比
# 为了提高反应效率，fe_dm.dm_sc_shelf_stock_daily 的存储过程放在了sh_prewarehouse_coverage_rate
SET @low_stock_rate = 
(SELECT ROUND(AVG(t.low_stock_rate),4) AS low_stock_rate
FROM 
(SELECT t3.sdate,
SUM(CASE WHEN b.grade_cur_month IN ('甲','乙') AND b.shelf_type IN (1,3) AND t3.stock_quantity < 180 THEN 1
WHEN b.grade_cur_month IN ('甲','乙') AND b.shelf_type IN (2,5) AND t3.stock_quantity < 100 THEN 1
WHEN b.ACTIVATE_TIME >= DATE_SUB(@sdate3,INTERVAL 60 DAY) AND b.shelf_type IN (1,3) AND t3.stock_quantity < 180 THEN 1
WHEN b.ACTIVATE_TIME >= DATE_SUB(@sdate3,INTERVAL 60 DAY) AND b.shelf_type IN (2,5) AND t3.stock_quantity < 100 THEN 1
WHEN b.grade_cur_month IN ('丙','丁') AND b.shelf_type IN (1,3) AND t3.stock_quantity < 110 THEN 1
WHEN b.grade_cur_month IN ('丙','丁') AND b.shelf_type IN (2,5) AND t3.stock_quantity < 90 THEN 1
WHEN b.grade_cur_month IN ('甲','乙','丙','丁') AND t3.stock_quantity < 300 THEN 1
END)/COUNT(*) AS low_stock_rate
FROM 
-- SELECT a.shelf_id,b.ACTIVATE_TIME,b.SHELF_TYPE,b.grade_cur_month AS grade
-- FROM 
...
-- FROM fe_dm.dm_op_shelf_grade a
-- WHERE a.month_id = DATE_FORMAT(@sdate3,"%Y-%m")
-- ) t2
-- ON t1.shelf_id = t2.shelf_id
-- JOIN fe_dm.dm_sc_shelf_stock_daily t3
JOIN fe_dwd.dwd_shelf_day_his t3
ON a.shelf_id = t3.shelf_id
AND t3.sdate >= @sdate2  AND t3.sdate <= @sdate3
GROUP BY t3.sdate) t
);
DELETE FROM fe_dm.dm_sc_preware_kpi WHERE sdate = @sdate3;
INSERT INTO fe_dm.dm_sc_preware_kpi
(sdate,
 seq,
 kpi_type,
 kpi,
...
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_sc_preware_kpi','dm_sc_preware_kpi','吴婷');
COMMIT;
    END