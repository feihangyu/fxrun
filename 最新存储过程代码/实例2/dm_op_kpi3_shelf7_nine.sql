CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_op_kpi3_shelf7_nine`()
BEGIN
  SET @run_date := CURRENT_DATE(),@user := CURRENT_USER(),@stime := CURRENT_TIMESTAMP();
-- 注意，如果当天跑数失败，不允许隔天补数。因为库存表只有前一天的数据报错
 SET @sdate := subdate(current_date,interval 1 day), @add_user := CURRENT_USER, @timestamp := CURRENT_TIMESTAMP;
  DROP TEMPORARY TABLE IF EXISTS fe_dm.nuse_tmp;
  CREATE TEMPORARY TABLE fe_dm.nuse_tmp
  SELECT
  ...
  CREATE TEMPORARY TABLE fe_dm.fill_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
    t.shelf_id, COUNT(DISTINCT t.order_id) filltimes, SUM(t.actual_fill_num) fill_qty, SUM(
      t.actual_fill_num * t.purchase_price
    ) fill_val, SUM(
      t.actual_fill_num * p.sale_price
    ) fill_sval, SUM(
      IF(
        t.fill_type IN (5, 6, 11), t.actual_fill_num, 0
      )
    ) fillout_qty, SUM(
      IF(
        t.fill_type IN (5, 6, 11), t.actual_fill_num * t.purchase_price, 0
      )
    ) fillout_val, SUM(
      IF(
        t.fill_type IN (5, 6, 11), t.actual_fill_num * p.sale_price, 0
      )
    ) fillout_sval
  FROM
  fe_dwd.dwd_fill_day_inc_recent_two_month t
    JOIN fe_dm.price_tmp p
  ...
  DROP TEMPORARY TABLE IF EXISTS fe_dm.shelf_inf_tmp;
  CREATE TEMPORARY TABLE fe_dm.shelf_inf_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
    t.shelf_id, f.firstfill activate_date, DATE(IFNULL(t.revoke_time, @add_day)) revoke_date, b.business_name, t.is_test
  FROM
    fe_dm.shelf_tmp t
    LEFT JOIN fe_dm.dm_op_shelf_firstfill f
  ...
  DROP TEMPORARY TABLE IF EXISTS fe_dm.cost_tmp;
  CREATE TEMPORARY TABLE fe_dm.cost_tmp (PRIMARY KEY (shelf_id, product_id)) AS
  SELECT
    s.shelf_id, p.product_id, t.purchase_price
  FROM
    fe_dwd.dwd_monthly_manual_purchase_price_insert t  -- 0519该表暂时没有数据，待唐进确定
    JOIN fe_dm.shelf_inf_tmp s
  ...
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_sale_day
  WHERE sdate = @sdate;
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_shelf_sale_day (
  ...
  FROM
    (SELECT
      t.order_id, t.shelf_id, t.user_id, t.order_status, t.product_total_amount, t.discount_amount, 
	  IFNULL(t.coupon_amount, 0) coupon_amount, 
	  SUM(t.quantity) quantity, SUM(t.quantity * t.sale_price) gmv, 
	  SUM(t.quantity_shipped) quantity_shipped, SUM(
        t.quantity_shipped * t.sale_price
      ) gmv_shipped, SUM(t.discount_amount) oi_discount_amount, SUM(t.real_total_price) oi_real_total_price, SUM(
        t.discount_amount * t.quantity_shipped / t.quantity
      ) oi_discount_amount_shipped, SUM(
        t.real_total_price * t.quantity_shipped / t.quantity
      ) oi_real_total_price_shipped, SUM(
        t.quantity_shipped * t.sale_price
      ) / SUM(t.quantity * t.sale_price) rate_shipped
    FROM
  ...
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_sale_week
  WHERE @week_flag
    AND sdate = @sdate;
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_shelf_sale_week (
  ...
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_sale_day t
  WHERE @week_flag
    AND t.sdate >= @week_start
    AND t.sdate < @add_day
  GROUP BY t.shelf_id;
  
  DROP TEMPORARY TABLE IF EXISTS fe_dm.oi_month_tmp;
  CREATE TEMPORARY TABLE fe_dm.oi_month_tmp AS
  SELECT DISTINCT
  ...
  DROP TEMPORARY TABLE IF EXISTS fe_dm.oi_month_user_tmp;
  CREATE TEMPORARY TABLE fe_dm.oi_month_user_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
    t.shelf_id, COUNT(DISTINCT t.user_id) users, COUNT(
      DISTINCT IF(t.order_status = 6, NULL, t.user_id)
    ) users_shipped
  FROM
    fe_dm.oi_month_tmp t
  WHERE ! ISNULL(t.shelf_id)
  GROUP BY t.shelf_id;
  
  
  DROP TEMPORARY TABLE IF EXISTS fe_dm.oi_month_sku_flag_tmp;
  CREATE TEMPORARY TABLE fe_dm.oi_month_sku_flag_tmp AS
  SELECT DISTINCT
    t.shelf_id, t.product_id, 'sal' flag
  FROM
    fe_dm.oi_month_tmp t;
  INSERT INTO fe_dm.oi_month_sku_flag_tmp (shelf_id, product_id, flag)
  SELECT
...
AND t.stock_quantity > 0;		
  ...
  DROP TEMPORARY TABLE IF EXISTS fe_dm.oi_month_sku_tmp;
  CREATE TEMPORARY TABLE fe_dm.oi_month_sku_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
    t.shelf_id, SUM(t.flag = 'sal') skus_sal, COUNT(DISTINCT t.product_id) skus_salsto
  FROM
    fe_dm.oi_month_sku_flag_tmp t
  WHERE ! ISNULL(t.shelf_id)
  GROUP BY t.shelf_id;
  ...
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_sale_month
  WHERE @month_flag
    AND sdate = @sdate;
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_shelf_sale_month (
    month_id, sdate, shelf_id, orders, users, product_total_amount, discount_amount, coupon_amount, orders_shipped, users_shipped, product_total_amount_shipped, discount_amount_shipped, coupon_amount_shipped, quantity, gmv, quantity_shipped, gmv_shipped, oi_discount_amount, oi_real_total_price, oi_discount_amount_shipped, oi_real_total_price_shipped, add_user
  )
  SELECT
  ...
  FROM
    ...
    LEFT JOIN fe_dm.oi_month_user_tmp u
      ON t.shelf_id = u.shelf_id;
	  
	  
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_fill_nday
  WHERE sdate = @sdate;
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_fill_nday (sdate, shelf_id, add_user)
  SELECT
    @sdate sdate, t.shelf_id, @add_user add_user
  FROM
    fe_dm.shelf_inf_tmp t;
	
	
DROP TEMPORARY TABLE IF EXISTS fe_dm.sf_product_fill_order_tmp;
CREATE TEMPORARY TABLE fe_dm.sf_product_fill_order_tmp (PRIMARY KEY (ORDER_ID,APPLY_TIME)) AS	
SELECT DISTINCT 
a.ORDER_ID,
a.APPLY_TIME,
a.send_time,
...
WHERE a.fill_time >= @sdate
AND a.fill_time < @add_day;
  
 CREATE INDEX idx_sf_product_fill_order_tmp
ON fe_dm.sf_product_fill_order_tmp  (shelf_id);
 
	
  UPDATE
    fe_dm.dm_op_kpi3_shelf7_fill_nday t
    JOIN
      (SELECT
        t.shelf_id, COUNT(*) forders, SUM(t.send_time < @sub_day) forders_ot, 
		SUM(t.send_time < @sub_day_adjweek) forders_ot_adjweek, 
		SUM(t.send_time < @sub_day_adjwork) forders_ot_adjwork
      FROM
        fe_dm.sf_product_fill_order_tmp t
        JOIN fe_dm.shelf_inf_tmp s
          ON t.shelf_id = s.shelf_id
      GROUP BY t.shelf_id
      HAVING forders > 0) f
      ON t.shelf_id = f.shelf_id 
	  SET t.forders = f.forders, t.forders_ot = f.forders_ot, 
	  t.forders_ot_adjweek = f.forders_ot_adjweek, 
	  t.forders_ot_adjwork = f.forders_ot_adjwork
  WHERE t.sdate = @sdate;
  ...
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_stock_his
  WHERE sdate = @add_day;
  
  
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_shelf_stock_his (
  sdate,
  shelf_id,
  sto_qty,
  psto_qty,
...
SELECT 
  ...
  SUM(t.stock_quantity * p.sale_price) sto_val,
  SUM(
    IF(
      t.stock_quantity > 0,
      t.stock_quantity * p.sale_price,
      0
    )
  ) psto_val,
  @add_user add_user 
...
HAVING sto_qty != 0 
  ...
  -- 取退款申请时间 并完成退款的。
DROP TEMPORARY TABLE IF EXISTS fe_dm.refund_tmp;
 CREATE TEMPORARY TABLE fe_dm.refund_tmp (PRIMARY KEY (shelf_id)) AS
 SELECT
   t.shelf_id, SUM(t.refund_amount) refund_amount
 FROM
   fe_dwd.dwd_pub_order_item_recent_one_month t
 WHERE  t.apply_time >= @sdate
   AND t.apply_time < @add_day
   AND ! ISNULL(t.shelf_id)
 GROUP BY t.shelf_id;
  
  
  DROP TEMPORARY TABLE IF EXISTS fe_dm.after_tmp;
  CREATE TEMPORARY TABLE fe_dm.after_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
  ...
  DROP TEMPORARY TABLE IF EXISTS fe_dm.check_tmp;
  CREATE TEMPORARY TABLE fe_dm.check_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
    t.shelf_id, SUM(
      t.audit_error_num * t.sale_price
    ) audit_lost_val
  FROM
  fe_dwd.dwd_check_base_day_inc t
    JOIN fe_dm.shelf_tmp s
      ON t.shelf_id = s.shelf_id
  WHERE t.operate_time >= @sdate
    AND t.operate_time < @add_day
    AND ! ISNULL(t.shelf_id)
	AND t.audit_status = 2
      AND t.error_reason IN (1, 2, 4)
  GROUP BY t.shelf_id
  HAVING audit_lost_val != 0;
  
  
  
  DROP TEMPORARY TABLE IF EXISTS fe_dm.fault_tmp;
  CREATE TEMPORARY TABLE fe_dm.fault_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
    t.shelf_id, COUNT(*) fault, SUM(t.fault_type = 4) fault4
  FROM
    fe_dwd.dwd_shelf_machine_fault t
  WHERE t.report_time >= @sdate
    AND t.report_time < @add_day
    AND ! ISNULL(t.shelf_id)
  GROUP BY t.shelf_id;
  
  
  DROP TEMPORARY TABLE IF EXISTS fe_dm.sku_flag_tmp;
  CREATE TEMPORARY TABLE fe_dm.sku_flag_tmp AS
  SELECT DISTINCT
    t.shelf_id, t.product_id, 'sal' flag
  FROM
  fe_dwd.dwd_pub_order_item_recent_one_month t
...
-- 取前一天的有库存的    
INSERT INTO fe_dm.sku_flag_tmp (shelf_id, product_id, flag) 
SELECT 
  t.shelf_id,
  t.PRODUCT_ID,
  'sto' flag 
...
WHERE t.stock_quantity> 0 ;
  ...
  CREATE TEMPORARY TABLE fe_dm.sku_tmp (PRIMARY KEY (shelf_id)) AS
  SELECT
    t.shelf_id, SUM(t.flag = 'sal') skus_sal, COUNT(DISTINCT t.product_id) skus_salsto
  FROM
    fe_dm.sku_flag_tmp t
  WHERE ! ISNULL(t.shelf_id)
  GROUP BY t.shelf_id;
  
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_stat_day
  WHERE sdate = @sdate;
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_shelf_stat_day (
  ...
  FROM
    fe_dm.shelf_tmp t
    LEFT JOIN fe_dm.dm_op_kpi3_shelf7_shelf_stock_his sto1
      ON t.shelf_id = sto1.shelf_id
      AND sto1.sdate = @add_day
    LEFT JOIN fe_dm.dm_op_kpi3_shelf7_shelf_stock_his sto0
      ON t.shelf_id = sto0.shelf_id
      AND sto0.sdate = @sdate
    LEFT JOIN fe_dm.fill_tmp fill
      ON t.shelf_id = fill.shelf_id
    LEFT JOIN fe_dm.dm_op_kpi3_shelf7_shelf_sale_day sal
      ON t.shelf_id = sal.shelf_id
      AND sal.sdate = @sdate
    LEFT JOIN fe_dm.refund_tmp ref
      ON t.shelf_id = ref.shelf_id
    LEFT JOIN fe_dm.after_tmp aft
      ON t.shelf_id = aft.shelf_id
    LEFT JOIN fe_dm.check_tmp ck
      ON t.shelf_id = ck.shelf_id
    LEFT JOIN fe_dm.sku_tmp sku
      ON t.shelf_id = sku.shelf_id
    LEFT JOIN fe_dm.fault_tmp ft
  ...
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_stat_week
  WHERE @week_flag
    AND sdate = @sdate;
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_shelf_stat_week (
  ...
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_stat_day t
  WHERE @week_flag
    AND t.sdate >= @week_start
    AND t.sdate < @add_day
  GROUP BY t.shelf_id;
  ...
  DELETE
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_stat_month
  WHERE @month_flag
    AND sdate = @sdate;
  INSERT INTO fe_dm.dm_op_kpi3_shelf7_shelf_stat_month (
  ...
  FROM
    ...
    WHERE @month_flag
      ...
      AND t.sdate < @add_day
    GROUP BY t.shelf_id) t
    LEFT JOIN fe_dm.oi_month_sku_tmp s
      ON t.shelf_id = s.shelf_id;
	  
	  
  DROP TEMPORARY TABLE IF EXISTS fe_dm.shelf_day_cost_tmp;
  CREATE TEMPORARY TABLE fe_dm.shelf_day_cost_tmp (PRIMARY KEY (sdate, shelf_id)) AS
  SELECT
    DATE(t.pay_date) sdate, t.shelf_id, SUM(t.quantity * c.purchase_price) gmv_cost, SUM(
      t.quantity_shipped * c.purchase_price
    ) gmv_cost_shipped
  FROM
     fe_dwd.dwd_pub_order_item_recent_one_month t
    JOIN fe_dm.cost_tmp c
  ...
  UPDATE
    fe_dm.dm_op_kpi3_shelf7_shelf_stat_day t
    JOIN fe_dm.shelf_day_cost_tmp c
      ON t.sdate = c.sdate
  ...
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_sale_week t
    JOIN fe_dm.shelf_inf_tmp s
      ON t.shelf_id = s.shelf_id
      AND ! s.is_test
  WHERE @week_flag
  ...
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_sale_month t
    JOIN fe_dm.shelf_inf_tmp s
      ON t.shelf_id = s.shelf_id
      AND ! s.is_test
  WHERE @month_flag
  ...
  FROM
    fe_dm.dm_op_kpi3_shelf7_shelf_stat_week t
    JOIN fe_dm.shelf_inf_tmp s
      ON t.shelf_id = s.shelf_id
      AND ! s.is_test
  WHERE @week_flag
  ...
  SET @sql_str := CONCAT(
	...
	"('d','自然日日机均GMV',@indicate_value_d1),('w','自然日日机均GMV',@indicate_value_w1),
	('m','自然日日机均GMV',@indicate_value_m1),", "('d','实收',@indicate_value_d2),
	('w','实收',@indicate_value_w2),('m','实收',@indicate_value_m2),", 
	"('d','有销售设备占比',@indicate_value_d3),('w','有销售设备占比',@indicate_value_w3),
	('m','有销售设备占比',@indicate_value_m3),", "('d','日均GMV>65元设备数',@indicate_value_d4),
	('w','日均GMV>65元设备数',@indicate_value_w4),('m','日均GMV>65元设备数',@indicate_value_m4),", 
	"('d','0销设备数',@indicate_value_d5),('w','0销设备数',@indicate_value_w5),
	('m','0销设备数',@indicate_value_m5),", "('d','低销设备数',@indicate_value_d6),
	('w','低销设备数',@indicate_value_w6),('m','低销设备数',@indicate_value_m6),",
	"('d','订单故障率',@indicate_value_d7),('w','订单故障率',@indicate_value_w7),
	('m','订单故障率',@indicate_value_m7),", "('d','销量故障率',@indicate_value_d71),
	('w','销量故障率',@indicate_value_w71),('m','销量故障率',@indicate_value_m71),", 
	"('d','折扣金额',@indicate_value_d8),('w','折扣金额',@indicate_value_w8),
	('m','折扣金额',@indicate_value_m8),", "('d','用券',@indicate_value_d9),
	('w','用券',@indicate_value_w9),('m','用券',@indicate_value_m9),", 
	"('d','自动贩卖机投放数量',@shelf_add_day),('w','自动贩卖机投放数量',@shelf_add_week),
	('m','自动贩卖机投放数量',@shelf_add_month),", "('d','次日上架率',@fill_not_rate_day),
	('w','次日上架率',@fill_not_rate_week),('m','次日上架率',@fill_not_rate_month),", 
	"('m','销售提成',@indicate_value_sm6),", "('m','商品成本',@indicate_value_sm7),",
	"('d','退款率',@indicate_value_sd1),('w','退款率',@indicate_value_sw1),
	('m','退款率',@indicate_value_sm1),", "('d','上架费',@indicate_value_sd2),
	('w','上架费',@indicate_value_sw2),('m','上架费',@indicate_value_sm2),", 
	"('d','货损金额',@indicate_value_sd3),('w','货损金额',@indicate_value_sw3),
	('m','货损金额',@indicate_value_sm3),", "('d','盗损金额',@indicate_value_sd4),
	('w','盗损金额',@indicate_value_sw4),('m','盗损金额',@indicate_value_sm4),", 
	"('d','盗损率',@indicate_value_sd5),('w','盗损率',@indicate_value_sw5),
	('m','盗损率',@indicate_value_sm5);"
  );
...
'dm_op_kpi3_shelf7_nine',
DATE_FORMAT(@run_date, '%Y-%m-%d'),
CONCAT('李世龙@', @user),
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_fill_nday','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_monitor','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_shelf_sale_day','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_shelf_sale_month','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_shelf_sale_week','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_shelf_stat_day','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_shelf_stat_month','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_shelf_stat_week','dm_op_kpi3_shelf7_nine','李世龙');
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_kpi3_shelf7_shelf_stock_his','dm_op_kpi3_shelf7_nine','李世龙');
COMMIT;
    END