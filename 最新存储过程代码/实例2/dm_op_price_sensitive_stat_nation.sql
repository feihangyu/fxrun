CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_op_price_sensitive_stat_nation`()
BEGIN
  ...
  DELETE
  FROM
    fe_dm.dm_op_price_sensitive_stat_nation
  WHERE week_end = @week_end;
  INSERT INTO fe_dm.dm_op_price_sensitive_stat_nation (
    week_end,
    product_id,
    salqty,
    gmv,
  ...
  FROM
    fe_dm.dm_op_price_sensitive_stat_nation t
  WHERE t.week_end = @week_end;
  UPDATE
    fe_dm.dm_op_price_sensitive_stat_nation t
...
@stime);
-- 记录表的数据量
CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_price_sensitive_stat_nation','dm_op_price_sensitive_stat_nation','李世龙');
