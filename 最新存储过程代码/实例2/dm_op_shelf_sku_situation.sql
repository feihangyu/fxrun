CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_op_shelf_sku_situation`()
BEGIN
...
LEFT JOIN fe_dwd.dwd_pub_product_dim_sserp p ON s.business_name = p.business_area AND d.product_id = p.product_id   -- zs_product_dim_sserp
GROUP BY shelf_id;
-- 每日更新并截存
...
SELECT @sdate sdate,
       ...
       -- t.normal_sto_val,
       t.out_sto_val,
       CASE WHEN (s.shelf_type IN (6,7) AND fill_sku < 15) 
            OR ((s.shelf_type IN (1,3,8) AND fill_sku < 25) 
            OR (s.relation_flag = 1 AND fill_sku <= 25) 
            OR (s.shelf_type IN (2,5) AND fill_sku < 10)) THEN 'sku偏少'
            WHEN ((s.relation_flag = 1 AND fill_sku > 25) 
            OR (s.type_name LIKE '%静态%' AND fill_sku >= 15 AND fill_sku <= 22) 
            OR ((s.type_name LIKE '%动态%' OR s.shelf_type = 7) AND fill_sku >= 15 AND fill_sku <= 30) 
            OR (s.shelf_type IN (1,3,8) AND fill_sku >= 25 AND fill_sku <= 55) 
            OR (s.shelf_type IN (2,5) AND fill_sku >= 10 AND fill_sku <= 20)) THEN 'sku正常'
...
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_op_shelf_sku_situation','dm_op_shelf_sku_situation','朱星华');
  COMMIT;	
END