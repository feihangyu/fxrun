CREATE DEFINER=`shprocess`@`%` PROCEDURE `dm_pub_shelf_grade`()
BEGIN
...
DELETE FROM fe_dm.dm_pub_shelf_grade
...
-- 给临时表添加索引
CREATE INDEX idx_tmp_shelf_id ON fe_dm.shelf_tmp (shelf_id);
...
CREATE TEMPORARY TABLE fe_dm.shelf_sale_tmp AS 
...
LEFT JOIN -- 货架销售数据
    (                                                                       
     SELECT shelf_id,                                                       
            SUM(gmv)gmv,
            SUM(ifnull(AFTER_PAYMENT_MONEY,0.00))payment_money                                 
...
-- 给临时表添加索引
CREATE INDEX idx_tmp_main_shelf_id1 ON fe_dm.shelf_sale_tmp (main_shelf_id);
-- 关联货架合计GMV、补付款
DROP TEMPORARY TABLE IF EXISTS fe_dm.relate_shelf_sale_tmp;
CREATE TEMPORARY TABLE fe_dm.relate_shelf_sale_tmp AS  
SELECT main_shelf_id,
       COUNT(*)shelf_amount,
       SUM(gmv_sum)gmv_sum,
       SUM(predict_gmv)predict_gmv
FROM fe_dm.shelf_sale_tmp p
GROUP BY main_shelf_id;
-- 给临时表添加索引
CREATE INDEX idx_tmp_main_shelf_id ON fe_dm.relate_shelf_sale_tmp (main_shelf_id);
-- 货架明细
DROP TEMPORARY TABLE IF EXISTS fe_dm.shelf_total_tmp;
CREATE TEMPORARY TABLE fe_dm.shelf_total_tmp AS  
SELECT p.smonth,                                                                                                                   
       p.business_name,                                                                                                            
       p.shelf_id,                                                                                                                 
       p.shelf_name,                                                                                                               
...
FROM fe_dm.shelf_sale_tmp p
LEFT JOIN fe_dm.relate_shelf_sale_tmp s ON p.main_shelf_id = s.main_shelf_id;
INSERT INTO fe_dm.dm_pub_shelf_grade
(month_id,                              
 business_name,                       
 shelf_id,                            
 shelf_name,                          
...
@stime);
-- 记录表的数据量
-- CALL sh_process.dwd_count_process_aim_table_size('fe_dm.dm_pub_shelf_grade','dm_pub_shelf_grade','朱星华');
  COMMIT;	
END