    ...
    try:
        sql='''
            ...
            CASE WHEN b.process_name IS NOT NULL AND b.create_time<>a.update_time THEN '修改'
                 ...
                 END AS flag
            FROM feods.`prc_procedure_detective_info` a
        ...
        for r in res:
            if r[4]=='新增':
                ...
                add_sql = '''
                -- 将新增的存储过程写入数据库
                INSERT INTO feods.`prc_procedure_detective_info`(procedure_name,create_time,update_time)
                SELECT process_name,create_time,create_time FROM test.prc_process_modify_info WHERE process_name='{}';'''.format(r[2])
                cursor.execute(add_sql)
                conn.commit()
                print('新增存储过程已提交数据库')
                title = '实例1存储过程新增通知'
                content = "组内同事，你们好！数据组于 %s 在实例1新增存储过程：%s，请对应维护人员及时告知组内维护人员，以便后续维护，请知悉！" % (r[3], r[2])
                sendEmail(title, content, email_list)
                print('存储过程新增通知已通过邮件发送')
                # 将操作日志信息写入数据库
                sql3e = "INSERT INTO feods.prc_procedure_exe_log(sdate,procedure_name,action_flag,loginfo) VALUES('{}','{}','新增','{}')" \
            ...
            if r[4]=='修改':
                ...
                sql4 = '''
                    ...
                    UPDATE feods.`prc_procedure_detective_info` a
                ...
                #查询维护人信息
                # 数据库查询维护人员及邮箱
                sql5 = "select process_name,maintainer,email from feods.prc_project_process_info " \
                ...
                # 发送邮件到指定的维护人员
                    title = '实例1存储过程修改通知'
                    content = "%s，你好！实例1上你维护的存储过程：%s 于 %s 发生过修改，请记得测试，确保准确无误！如是本人操作，请忽略；非本人操作，请咨询组内人员！" % (
                    maintainer, r[0], r[3])
                    sendEmail(title, content, email)
                    print('存储过程修改通知已通过邮件发送')
                # 将操作日志信息写入数据库
                sql4e = "INSERT INTO feods.prc_procedure_exe_log(sdate,procedure_name,maintainer,action_flag,loginfo) VALUES('{}','{}','{}','修改','{}')" \
            ...
            if r[4]=='删除':
                ...
                # 将删除信息写入数据库
                print('将删除时间写入数据库')
                sql8 = "update feods.prc_procedure_detective_info set delete_time=NOW(),delete_flag=1 where procedure_name = '{}' and delete_flag=0 ".format(r[0])
                cursor.execute(sql8)
                conn.commit()
                ...
                sql10 = "SELECT a.project,b.maintainer,CONCAT(a.update_frequency,a.start_time,'开始执行') AS frequecy,a.dependent_project " \
                        "FROM feods.`prc_project_relationship_info` a JOIN (SELECT project,maintainer,email " \
                        "FROM feods.`prc_project_process_info` GROUP BY project,maintainer,email )b " \
                ...
                # 获取失败任务的维护人员 及邮箱
                sql7 = "select process_name,maintainer,email from feods.prc_project_process_info " \
                       "where process_name='{}' group by process_name,maintainer,email ".format(r[0])
                cursor.execute(sql7)
                conn.commit()
                res = cursor.fetchall()
                if res:
                    ...
                    content = "%s，你好！实例1上你维护的存储过程：%s 于 %s 被删除，将会对以下任务有影响：\n%s\n以上，请知悉！\n如是本人操作，请忽略；非本人操作，请咨询组内人员！" % (
                        maintainer, r[0], del_time, table_list)
                else:
                    maintainer = ''
                    email = []
                    del_time = datetime.now()
                    content = "组内同事，大家好！实例1上未知人员维护的存储过程：%s 于 %s 被删除，将会对以下任务有影响：\n%s\n以上，请知悉！\n请该操作人员及时告知组内维护人员，谢谢！" % (
                ...
                # 将操作日志信息写入数据库
                sql8e = "INSERT INTO feods.prc_procedure_exe_log(sdate,procedure_name,maintainer,action_flag,loginfo) VALUES('{}','{}','{}','删除','{}')" \
                    .format(date.today(), r[0], maintainer, content)
                cursor.execute(sql8e)
                conn.commit()
                print('操作日志信息已写入数据库')
        #更新feods.prc_procedure_exe_log 表的详细操作时间数据
        update_sql = "UPDATE feods.`prc_procedure_exe_log` SET action_datetime= CASE WHEN action_flag='修改' THEN LEFT(SUBSTRING_INDEX(SUBSTRING_INDEX(loginfo,'于 ',-1),'发生',1),19) " \
