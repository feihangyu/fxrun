...
def createTable(conn,cursor):
    try:
        ...
        sql = '''
                ...
                ) ENGINE=INNODB DEFAULT CHARSET=utf8mb4 COMMENT='dwd_dm库表最新创建信息'
            '''
        cursor.execute(sql)
        conn.commit()
...
def insertData(conn,cursor):
    ...
    FROM `information_schema`.`TABLES` WHERE TABLE_SCHEMA IN ('fe_dm','fe_dwd')
    ...
    try:
        ...
        if num>0:
            for r in res:
                ...
                LEFT JOIN (SELECT project,update_frequency FROM fe_dwd.`dwd_prc_project_relationship_detail_info` GROUP BY project,update_frequency)b
                ON a.project=b.project
                LEFT JOIN (SELECT project,maintainer,email FROM  fe_dwd.`dwd_prc_project_process_info` GROUP BY project,maintainer,email) c
                ...
                if res:
                    wy_list = '\n'.join(list(set(['将会影响：'+i[3] + '的'+i[1]+ '网易有数的抽取任务：'+ i[2] for i in res])))
                    if wy_list =='':  #以防上面table_list为空
                        wy_list = '通过网易有数抽取sql查询，对其他网易有数抽取任务没有影响'
                content = "实例2表：%s 被删除，将会对以下任务有影响：\n%s\n%s" % (del_table, table_list,wy_list)
